...
Перевірено 2026-09-09 на власному столі власника. Smart App Control
блокує непідписані файли, і кожна нова збірка для нього новий файл, тож
кожна стартує заблокованою. Винятку для окремої програми немає. Вимкнути
його можна в «Безпека Windows» → «Керування програмами та браузером» →
«Параметри Smart App Control», і з квітня 2026 там само ввімкнути назад.
Значення в реєстрі оновлюється із запізненням, орієнтуйся на те, чи
запускається програма, а не на нього.
...
```powershell
& "$env:USERPROFILE\Downloads\prowlone-build-0.9.4\ProwlOne_0.9.4_x64-setup.exe" /S
```
...
- **Для тестувальників** цей шлях не годиться. Або підпис (безкоштовно
  через SignPath, потрібні публічні релізи на GitHub; рішення ще не
  прийняте), або Windows-збірку давати лише тим, у кого Windows 10 або
  вимкнений Smart App Control, і чесно писати про це в оголошенні.
