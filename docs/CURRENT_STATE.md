...
Earlier — **2026-09-18, evening — stage 2, step 1 is on `main` and judged unusable by the owner at the screen: «куча багів, все нелогічно, щоб щось зробити потрібно мати IQ 130». Work stopped for the day; tomorrow begins with his list of what was illogical, and the flow is reworked before anything else of stage 2.** What the day built is described below and is not withdrawn; what it lacks is a walk-through in the real window by the person who has to use it - the tests and the browser demo passed, the owner's hands did not. The two bands (service mode, bench) were already withdrawn on his word the same evening: the switch is red while the mode is on, the header badge carries the bench and its scenario in a circle.

Earlier — **2026-09-18 — stage 2, step 1 is built on the branch `stage-2`: the service mode and the clear of the codes (`ADR-0036`); the 1.1 line waits for a car.** The first operation that changes what a module holds is on the bench end to end: the mode switched on behind one consent, the clear offered only under codes read in this session, one question, `0x14` in the default session or in the extended one opened and left for it, the codes read again, the record with what was erased, the guard loosened for exactly that service. Not yet: the collective "clear every surveyed module". The release 1.2.0, to the owner alone, follows his review of the branch.

Earlier the same day — **stage 2 is authorised (`ADR-0036`).** The owner opened stage 2 on the plan put to him: four steps, one operation class at a time — clear the codes, run a routine, drive an actuator, change an adaptation — behind a service mode switched on per session, one confirmation per operation, the extended session opened and left, every operation recorded, the guard loosened one service at a time; 1.2.x installers to him alone, the testers' line on `release-1.1`. Nothing of it is built yet; the first commit of stage 2 is step 1, the clear.

Earlier — **2026-09-17 — the live view has its face, stage 1 is built, and the product turns 1.1.0; what remains is a car.** The version says where the product stands: the second number is the stage (1.1.x is stage 1 built, 1.2.x will be stage 2, 1.3.x stage 3), the third counts the builds within it — the owner's rule, recorded in ROADMAP.md. The visual layer of F15, the last row the capability matrix held open inside stage 1, was built and pushed: the live table as the one list, a row marked for a tile or for the one chart, rows at zero hidden until asked, the operator's own warning and alarm limits where SDD records no normal range, and over the table, on one button, an instrument panel after the car's own cluster — speedometer, tachometer, and between them what crossed a limit or the key readings. Around it, three corrections the owner's screen demanded: the bench writes a reading a car could show through the parameter's own converter instead of bytes that read 136 V on a 0…5 V channel (`ADR-0020`, amendment), units are symbols in the interface language rather than SDD's codes, and a reading carries the decimals of its converter's step. The owner's words for where this leaves the stage: «з нашого боку закрита умовна теорія. зараз чекаємо підтвердження практикою» — the theory is closed on our side, and what remains is confirmation by practice: the first tester's report (M6), the map-converter scale a tester's read would settle, and F14's second slice on an early Range Rover. The validation states are unchanged in kind: the multi-module read path has never met a car.

Earlier — **2026-09-16 — what a module will accept, listed and not sent (`ADR-0035`), and the product turns 1.0.** The owner opened the desk work of stage 2 before any tester's report exists — "переписати правий бік довідника, скласти ADR, розвести операції по класах і обкатати їх на стенді можна все зараз, за столом" — and the half of SDD's module catalogue this project had always left behind entered the knowledge base. A new adapter in `sdd-ingest` reads `ACCESS_PARAMETERS` and `ROUTINE_IDENTIFIERS` from the same `MDX_<ECU>.xml` documents the readable half already came from, and writes three namespaces of its own: `sdd_module_write`, `sdd_module_control`, `sdd_module_routine`. Measured with the parser over 1,790 documents, 53 programme-years and 102 kinds of module — 2,294 writeable, 124 controllable, and 8,144 routine entries over 253 distinct routine numbers; 628 writeable and 24 controllable stand behind a security level, and every writeable and controllable one names the extended session `session_03`, so stage 2 begins with `0x10 03` and with keeping that session alive rather than with the write itself. A first count taken with regular expressions was wrong on three of the four, and the ADR corrects it in the open rather than quietly. Seven documents carry a second `<DATA_IDENTIFIERS>` or `<ROUTINE_IDENTIFIERS>` section: the adapter reads every section, and the first export, which read only the first of each, lost two routines and said nothing about it. Each row carries its safety class — a write is `PERSISTENT_CHANGE`, a control `VOLATILE_CONTROL`, a routine `SERVICE_ROUTINE` — and the module panel lists them under what was read, with the service, the session, the security level and the run time SDD declares. **Nothing sends any of it**, and the bench now says so at run time: with the declarations loaded and on screen, a whole bench session — survey, reads, fault codes, passport, configuration, battery, odometer, live reading, standard OBD, the K-line modules, a capture and the report preview — asked the vehicle for exactly four services, `0x01`, `0x03`, `0x19` and `0x22`, and for none of `0x10`, `0x11`, `0x27`, `0x2E`, `0x2F`, `0x31`, `0x34`, `0x36`, `0x37` or `0x3E`. The request constructors stay absent, `SAFETY_BOUNDARIES.md` is unchanged, and the guard in `scripts/check-architecture.mjs` is untouched. The library was exported the same day from the eleven corpus roots — 11 bundles, 12,157 sources, 380,149 evidence records, 517,595 knowledge records, none rejected, with `module_access.json.gz` holding 1,764 manifests and 10,482 records — and issued to the owner as copy `A391-A466`, valid until 2027-09-16; the application was then checked against that folder under the issuing key. The instrument itself was finished in the same stretch: the sheet turned a light Jaguar green, every bezel and border went, the plate controls carry a 1 px white hairline, and the battery became the key that starts the reading, with its poles marked and the charge drawn in its cell. **Build 1.0.0** — the owner's decision to leave 0.9.x behind ("переходь замість 0 на 1 на першому місці") — came from `0899641` (CI run 34751963231; the run on the release commit itself never finished on the morning CI stalled): `ProwlOne_1.0.0_x64-setup.exe`, 5,683,534 bytes, SHA-256 `ec3e6c173bb4d74d92ebb6b8486a15dba4807ea017a870f3f216bf4b3d9f5507`; `ProwlOne_1.0.0_universal.dmg`, 15,432,616 bytes, SHA-256 `bcc640cb8811b4dcf797e5d6e03c00d6760d3db3373cd719817c6453053288eb`. **Build 1.0.1** followed on `db9afe8` (CI run 35075826103, all four jobs green), the first that shows what a module will accept: `ProwlOne_1.0.1_x64-setup.exe`, 5,706,533 bytes, SHA-256 `d739b3bb5a110da8cd28a7851a41afa53e7fd8c756bc297a72e259d318cb0465`; `ProwlOne_1.0.1_universal.dmg`, 15,474,011 bytes, SHA-256 `93944fe46bda13db91756ad1667b2e6f44e7f1b2c6b14bf5530b75a1f9e745ff` — in `Downloads\prowlone-build-1.0.1` and on the owner's desk with the library copy A391-A466. `IMPLEMENTED / FIXTURE_TESTED`; nothing here is `VEHICLE_CONFIRMED`, because no car has answered anything yet. Since the build: the folder remembered from last time no longer takes the choice away while it reads — it counts its seconds like any other reading, both buttons stay live, and a folder chosen meanwhile wins, with the slower answer of the one it replaced dropped instead of written over it.

Earlier — **2026-09-13 — the fault-code help is SDD's own text, in English or Russian (`ADR-0034`; supersedes `ADR-0026`).** The owner withdrew the translation layer: the 27,493 rows of `help_texts.tsv` and the module that read them left the product, and no help text is translated by anyone — not by hand, not by a machine, not later. What decided it, in order: a 14B model on the owner's card had turned *exhaust gas recirculation bypass solenoid circuit* into the turbine vane actuator with every code and number intact; the rows already written were then measured against SDD's own text for the first time — 99.2 % carried the same content words and none had swapped a component, but 56 had replaced a specific procedure (*check the neutral sensor, refer to the circuit diagrams, check the circuit, then suspect the powertrain control module*) with one generic sentence about confirming the diagnosis; and SDD 169 turned out to ship the whole layer in Russian, `COMMON_SDD_DATA_DTC_HELP_LANG_RU`, the same 6,171 documents and 42,637 named texts with only the words changed, a Russian text for every one of the 38,526 names the library's screens use. The owner's scripts opened that pack from the installer (every file verified against the cabinet's MD5, the installer's SHA-256 confirmed); the exporter now takes a `_LANG_RU` root and writes each screen's Russian beside its English — `sdd_help_screen.<NAME>.rus`, `sdd_help_screen_items.<NAME>.rus` — from a source of its own, contributing text and nothing else, and refusing a document whose own `<language isoCode>` is not the one it is read as. The session joins a Russian line to its English by the mnemonic's name, so a name the Russian screen lacks keeps English and the lists stay the screen's length; a library issued without the pack reads as it always did. The help panel and the self-test list carry one switch, **English or Russian**, remembered on the machine; a Ukrainian interface reads Russian by default. Ukrainian is not offered for this text. The owner opened the ODST Russian pack the same morning (`COMMON_SDD_DATA_ODST_LANG_RU`, the same 93 documents, `isoCode="ru"`), and the self tests follow the same path: the ODST adapter told the language writes `sdd_odst_screen.<NAME>.rus` and `sdd_odst_screen_items.<NAME>.rus`, the items claim now written for English too, and the session joins a test's Russian to its English line for line by name. Seven mnemonics in both packs carry their own name as their text (`J_HLP_ST_FCIMC_01`…); that is what SDD shows in both languages and it is left as SDD has it. **Later the same day the code descriptions followed** (the amendment to `ADR-0034`, on the owner's word): the Russian pack's `dtcDescriptions.xml` and `dtcModuleDescriptions.xml` enter as `sdd_dtc_description.rus` beside the English alias, checked by their words since the index declares no language; a described code carries `description_data_texts["rus"]` with the same scope as its English, and the code's line on screen follows the same English-or-Russian choice as the help — our own wording for a standard code first, SDD's description next, English last, the English kept beside. The failure type's wording follows that choice too. The library was re-exported the same day with the Russian root: 499,765 records over 10,298 sources, `dtc_help.json.gz` now 9,887 manifests — 6,168 English and 3,719 Russian, the 2,449 Russian documents whose screens no car selects producing no manifest rather than a rejection — with 36,890 screens in each language, every Russian screen's items in the same order as its English twin, and `rejected: 0`; the owner's copy was stamped **2E57-6A34**, and again the same morning as **6E07-030B** once the ODST Russian pack was in — `odst.json.gz` 186 manifests, 5,651 records, 462 screens in each language with every Russian screen's items in its English twin's order, the store at 501,331 records over 10,391 sources, `rejected: 0`; 29.2 MB zipped, valid to 2027-09-13 — and a third time as **B101-0674** with the Russian descriptions: `dtc_index.json.gz` 5 manifests, 11,828 records, 5,566 (code, module) pairs in Russian against 5,569 in English (7 and 10 without a twin, SDD's own asymmetry), the store at 507,113 records over 10,393 sources, `rejected: 0`, 29.5 MB zipped. **Build 0.9.9** was cut the same morning on `c8bdbaf` (CI run 34737814836, all four jobs green): `ProwlOne_0.9.9_x64-setup.exe`, 5,321,004 bytes, SHA-256 `99a29809fad11dd80d1d688f411864e00af8ab39e88ca5e76f317128377d40ec`; `ProwlOne_0.9.9_universal.dmg`, 14,708,795 bytes, SHA-256 `f73e43544548777072d2a651f2b8ffccb871b1b34e5cfd01182090de898ee484` — in `Downloads\prowlone-build-0.9.9` and on the owner's desk beside the stamped library. **Build 0.9.10** followed an hour later on `332deef` (CI run 34739212726, all four jobs green), the first that reads the self tests' Russian: `ProwlOne_0.9.10_x64-setup.exe`, 5,321,138 bytes, SHA-256 `59053ac168978ed8a65c99e22385a3b8dce2337aa3716e89e8e590ae2b02c006`; `ProwlOne_0.9.10_universal.dmg`, 14,717,174 bytes, SHA-256 `33e8292db7f9196874d5c669635bf327e6bd4d684227fb349d72471c1872ca1a` — in `Downloads\prowlone-build-0.9.10` and on the owner's desk with the library copy 6E07-030B. **Build 0.9.11** followed on `df09659` (CI run 34740651226, all four jobs green), the first that shows the code descriptions in Russian: `ProwlOne_0.9.11_x64-setup.exe`, 5,322,684 bytes, SHA-256 `92fbf2517bd03785df613267e82710c2737a3b6c6263383721f9de178f6657bd`; `ProwlOne_0.9.11_universal.dmg`, 14,718,117 bytes, SHA-256 `75178b754686138016c0a7978fe21987ce3a79923ac91a93059ec6ab9c487567` — in `Downloads\prowlone-build-0.9.11` and on the owner's desk with the library copy B101-0674. `IMPLEMENTED / FIXTURE_TESTED`. Not decided: SDD's Russian for the code descriptions, which the same pack carries.
...
bench under it. Two of the items reached the library and got their records;
the rest stayed in the interface.

### What was underneath

**The car's configuration could not be read, because SDD names its holders
by their diagnostic system.** The owner's X250 refused the configuration
read: the library declared no readable block for the `RSJB`, the `FSJB` or
the `PCM`, the holders of its three copies. Counted in his library: 335
records, all of them addresses of configuration blocks, are scoped not to a
module but to a diagnostic system of one — `RSJB_SYSTEM_A`, `PCM_SYSTEM_B` —
and every one of the six families named that way is also named bare
elsewhere. The read plans its route by the module, so a record scoped to the
system was applicable to nothing. `ADR-0037` gives the rule and its limits:
`_SYSTEM_` followed by one capital letter at the end of a name, and nothing
else, names the module before it; the module dimension of applicability, the
store's filter and index, the resolver's own match and the configuration's
sources all read it through one function, `knowledge::system_family`.
Written before the change, built the same hour. On the X250 through the same
library: sources `RSJB` (sync), `FSJB`, `PCM`; four blocks under `RSJB`
(`0xC25F`, `0xD900`, `0xF105`, `0xF106`), the same four under `FSJB`, two
under `PCM`; a layout of 629 parameters. Whether the car answers is
`VEHICLE_CONFIRMED` and unearned — the read plans now, where before it
refused.

**The X250's battery monitor has no voltage.** The owner chose a scenario
ending in nine on the bench and saw no warning. The battery read's headline
voltage, `0x402A`, is not among the thirteen battery parameters his library
declares for the `RSJB`, so the low-battery line — `ADR-0030`, amended in
the morning to make SDD's own low band, under 11.6 V, a precondition of the
session, said in one sentence under the header and repeated inside the
clear's confirmation — could never appear on his car, bench or road. Amended
again the same day: the voltage is the freshest of three readings, each with
its time — the battery read's headline, the legislated `0x42`, and a live
read of a module's supply voltage — and the bench's rail follows the
scenario on every one of them, not on the battery read alone: 11.2 V on
`0x402A`, on `0x42` and on a live supply on a scenario ending in nine, 12.6
and 14.0 V otherwise, a sensor channel keeping its band. Read through the
whole stack in a shell test. The adapter measures nothing itself, and the
record says so rather than inventing a read.

### What the owner named at the screen

- **the debt from yesterday** — the help-language switch does carry the self
  tests' text (24 of 24 on the X250 have Russian screens); what it cannot
  carry are the operations' names, which SDD writes in English only and the
  data holds in no other language (`ADR-0034`). A caption says so, and the
  names lost their weight;
- **the battery and the configuration go away on a button**, as the passports
  and the mileage already did, and each is one table now, with its groups as
  rows rather than a stack of tables;
- **the battery's fill is the phone's green** — a white at half strength on
  the pale plate was all but invisible;
- **the voltage is on the cell**, in bold, from whichever read last brought
  one, and gives way to the cell's word under the pointer;
- **the headings of the four long reads hold the top of the screen** while
  the read scrolls under them, so the button that puts a read away is at
  hand thirty rows down and not back at the head of the table;
- **the technical details of a failed read** sit behind one spoiler in each
  of the four panels, in the shape the self tests and the accepted operations
  already had;
- the probe and the stopwatch that measured yesterday's numbers are gone from
  `bench_e2e.rs` on the owner's word.
...
Then the owner surveyed an L322 and nothing had moved («L322 нічого з
недосяжного не стало доступним»), and he was right twice. The car was
of the DS2 era — the K-line lane on his map says so — whose ring
decision 3 leaves out. And the L322 of 2010 had not moved either:
surveyed on his library it had the route and no identifiers, because
the MY10 documents give a MOST module only `phys` 0x7A4 and no
`can_tx`/`can_rx`, and identifiers were derived from a physical address
only on the 29-bit `normal_fixed` buses of `ADR-0017`. Measured over
the 45 documents: 1,528 of 1,528 declared pairs answer eight above the
request, and exactly 31 rows — the MOST rows of the three MY10
documents — carry a physical address in the 11-bit range. Decision 6
derives them on the exporter's request, `Unverified`, citing the
measured convention beside the module's own address; the synthetic
`TVPHYS` proves it in the platform golden, and the synthetic survey
...
### The evening: step 1 accepted at the screen

The walk-through that 2026-09-18 ended without: the owner went through the
service mode and the clear of a module's codes in the running window and
accepted the flow — «ми виправили всі мої зауваження, все тепер логічно».
His remarks of the evening before were the interface's, not the
operation's, and the two days of rework answered them. Then, on his word,
the one thing the built note had left for a later commit: **the
collective clear** — under the list of what the car answered, one
question that names every module with codes and its count, then each
...
The morning began where `ADR-0036` left off: the owner's own list, at the
screen, in the order he found things. What follows is that list and what
each item turned out to be.

### What was underneath

**The bench had been reporting fault codes all along.** The owner went
through eight scenarios and saw none. Measured on his library through the
real read path: every non-zero scenario gives codes — 16 to 21 modules with
codes, 22 to 31 codes, no read failing. The check asks every reachable
module for its confirmed codes and kept each answer inside whichever module
was clicked afterwards, so a check that found two dozen looked exactly like
a check that found none. The map now has a list under it: how many codes in
how many modules, each module and its codes in the reader's own language,
and a plain count of what answered clean and what did not answer at all.

**The library was read once per module, not once per question.** The
knowledge store had no index: every query walked all 517,595 records. The
first fix, an index by module and by diagnostic implementation, changed
almost nothing, and the measurement said so honestly. The cost was not
finding the records but asking for all of them: `DiagnosticEnvironmentResolver::resolve`
requested the whole store and filtered it in Rust, and a survey resolves two
or three capabilities per module. It now names to the store the three things
`record_is_related` can match on — the module, the implementation, and the
entity — so the index has something to work with, and the filter still
decides. One query naming a module went from a share of a second to 5 ms.

| step, release build, the owner's library | before | after |
| --- | --- | --- |
| reading the library | 49 s | 47 s |
| surveying 37 modules | 55 s | 6.3 s |
| building the bench, first time | 65 s | 8.3 s |
| building the bench, same vehicle again | 65 s | instant |
| building the bench, another scenario | 83 s | 6.5 s |

The index has to be built as the library loads, and a first version that
allocated a key for every one of half a million records added forty seconds
to the load; written to allocate only for a new name, the load is unchanged.

**The bench was rebuilt on every connection**, over a vehicle the survey had
already built, which is what the owner watched take minutes. It is now built
when what it is built from changes — another vehicle, another scenario, a
new session — and not otherwise. And a bench connection while the survey is
running no longer waits for the session the survey holds, which used to hold
the adapter service with it and freeze the connection card: it is refused at
once with its reason (`SessionBusy`), and the interface does not offer the
button while a survey runs.

No ADR was written for any of this: no boundary moved, the architecture
guard is unchanged and passes, and what the application answers is the same.
The knowledge crate gained an index and a query filter; the contract gained
one error code.

### What the owner named at the screen

- **the bench badge had no colour** — it had been given one when the red
  bands were withdrawn, and three later rules for the plate's capsules,
  carrying the same weight and standing lower in the sheet, had painted over
  it ever since. It is red now, written at the foot of the sheet;
- **the connection said nothing while it worked** — the card fell through to
  its empty state and offered a button reading "detect adapter" for as long
  as a build took. It now names what is being connected and counts the
  seconds, as the library load and the survey already did;
- **the header moved when the language changed** — measured at the owner's
  1500-point window, the left edge of the row shifted 205 points between
  English and Russian, 141 of them from one badge that carried a whole
  sentence. The badges now name the thing and let the dot carry the state,
  every capsule reserves the width of its longest language, and the language
  switch stands outside the group that wraps. Measured after: identical
  geometry in all three languages at every width tried, 1500 down to 860. A
  test counts the labels against the reserved width so a future translation
  cannot quietly start the movement again;
- **"Identifier" named the address, not the thing at it** — the operation now
  says what it brings back, "one value the module holds", before the list is
  opened;
- **the long reads never left the screen** — the passports and the mileage
  are shown when the read arrives and go away on one button, the same rule
  for both;
- **the self tests and what a module accepts** are spoilers now, in the shape
  of the exchange above them;
- **one face, four weights** — a serif on one circular mark was the only
  other typeface in the sheet, and seventeen distinct weights had grown up in
  it. The scale is 400, 500, 600, 700, and emphasis is size;
- **the network actions** have the width of the panel to themselves, with the
  option that modifies one of them under them rather than in front of them,
  and the repeated verb is gone from three buttons of four.

### What this day did not touch

No operation has met a car; stage 2 step 1 is what `ADR-0036` describes and
nothing was added to it. The collective clear is still unbuilt. The library
load is still 47 seconds and is a different problem — parsing 12,151
manifests — left alone.

## 2026-09-17 — the live view has its face; stage 1 is built and waits for a car

What was built, in the order the owner asked for it — all in the interface
but for the bench and the decoder's formatting:

- **The table is the one list.** Tiles and the chart are marks on its rows —
  a plate like the language switch, each mark its own lens — with «mark all
  for the chart» and «clear marks». Rows that read zero and never moved are
  hidden behind one button that says the opposite of the current state, with
  a count of what is hidden. «Seen» became «Min … max».
- **One chart for the run.** Every marked parameter on the same axis of time,
  each scaled to its own span, no legend: the table row carries the colour.
- **Limits are the operator's.** SDD records no normal or display range for
  any parameter — checked across the library — so a tile colours itself only
  against warning and alarm limits the person sets, kept on the machine; one
  starter suggestion, the battery's 11.5…15 V, is marked as ours. The
  application states no threshold of its own.
- **The instrument panel.** On one button beside «show all», remembered on
  the machine: the speedometer to 260 km/h, the tachometer to 7 000 rpm,
  whole numbers in the face, a pointer and a progress arc on the rim, the
  person's alarm and warning limits as red and amber arcs; between them what
  crossed a limit, alarms first, or the key readings — coolant, supply
  voltage, oil, intake air, fuel pressure, boost; below, the outside
  temperature and the odometer when the run reads them. A dial reads the
  quantity itself (`PCM 0xDA48` engine speed, `PCM 0xDA02` vehicle speed),
  not a timeout log that mentions it.
- **Units as symbols.** The library's 29 SDD unit codes — `int` on 65 % of
  the 15,281 parameter records, then `V`, `pct`, `km`, `degC`, … — become
  symbols in the interface language; `int` is SDD's «no unit» and shows
  nothing; a code the table does not know is shown as it is. The CSV keeps
  the codes.
- **Decimals of the step.** A reading is written with the decimals its
  converter's step calls for, three at most, and keeps them on a whole
  number; the run's smallest and largest take the same.
- **The bench writes readings, not bytes.** The 136.8 V the owner saw was
  `PCM 0x1265`, a 10-bit channel at 5/1024 V a count in a 16-bit field, filled
  by the bench with 0x6D72 — 28 018 counts where a real converter never
  exceeds 1 023. Every parameter the catalogue places now gets a number a
  standing car with the engine running could show, through its own converter,
  wandering over 64 requests; the battery table's 0x402A is 132 counts,
  12.6 V, where 202 read 16.1 V (`ADR-0020`, amendment of 2026-09-17).
- **A failed bench call is the bench's failure**, in the interface language,
  not USB advice: the development window restarting under the owner's click
  had shown «Detect adapter» again.

Where stage 1 stands, in the owner's words: «з нашого боку закрита умовна
теорія. зараз чекаємо підтвердження практикою». Every function declared for
the stage is built; the capability matrix holds no row open inside it. What
closes the stage is not ours to build: the first tester's report (M6) with
the first `VEHICLE_CONFIRMED` records of the multi-module path, a tester's
read of one quantity through a linear and a map identifier for the
map-converter scale, and F14's second slice on the adapter and an early
Range Rover. Stage 2's desk work (`ADR-0035`) remains preparation, not
authorisation.

Recorded the same day for whoever works in the running development window:
a saved Rust file rebuilds and restarts the window under the person testing
it, and a click during the swap fails; interface files reload in place.
...
# — with the Russian packs mounted as well and their roots added, e.g. -v "<where the owner's scripts wrote them>/SDD_original_help/RU:/corpus_ru" -v ".../SDD_ODST_RU/original:/corpus_odst_ru" and /corpus_odst_ru/COMMON_SDD_DATA_ODST_LANG_RU (`ADR-0034`)
```
