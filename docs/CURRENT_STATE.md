...
Status: **2026-09-18 — stage 2, step 1 is built on the branch `stage-2`: the service mode and the clear of the codes (`ADR-0036`); the 1.1 line waits for a car.** The first operation that changes what a module holds is on the bench end to end: the mode switched on behind one consent, the clear offered only under codes read in this session, one question, `0x14` in the default session or in the extended one opened and left for it, the codes read again, the record with what was erased, the guard loosened for exactly that service. Not yet: the collective "clear every surveyed module". The release 1.2.0, to the owner alone, follows his review of the branch.
