...
| `live_read_runs` | `prowlone.live-read-run`, operation `LIVE_READ` | a module read repeated at a stated cadence, floor and cap in the shell; the same series can be exported as a spreadsheet, which is a rendering of the record and not a second one, and on the bench nothing is written to disk | `ADR-0022`, amended 2026-09-12 |
...
| `battery_reads` | `prowlone.battery-state`, operation `BATTERY_STATE` | one `ReadDataByIdentifier` per module and battery identifier the loaded data names — state of charge, current, estimated temperature, quiescent current, cumulative charge and discharge, time in service, monitor resets, cold cranking voltage, battery type, and the traction battery of a hybrid — each value the number the data describes or the bytes it does not; no threshold of this product is applied and nothing is written: battery registration and monitor reset stay out of stage 1, and `0x4020` is read as a count of them | `ADR-0030` |
...
| `routine_runs` | `prowlone.routine-run`, operation `ROUTINE_RUN` | one run of routine `0x0202`, the on-demand self test, on a module whose index declares it without a security level and for which the ODST pack gives this car the test with its time and timeout — the extended session opened first and held with `TesterPresent` while the test runs, `RoutineControl` start, the request for results once the data's time has run, the way back with whichever step ends the run (results, refusal, the person's stop, an error); the result is the bytes the module answered with, every exchange is in the record, and once the run has ended the module's codes are read again — both reads in the record, so what the test logged is seen | `ADR-0036`, step 2 |
